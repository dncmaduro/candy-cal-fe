...
import { type IncomeProductSource } from "../../hooks/models"
...
export const IncomeDetailPage = ({
  ...
  const calculations = useMemo(() => {
    ...
    const totalRevenue = income.products.reduce(
      (sum, item) => sum + (item.priceAfterDiscount || 0) * item.quantity,
      0
    )
    const totalDiscount = income.products.reduce(
      (sum, item) =>
        sum +
        ((item.platformDiscount || 0) + (item.sellerDiscount || 0)) *
          item.quantity,
      0
    )
    const totalBeforeDiscount = income.products.reduce(
      (sum, item) => sum + (item.quotation || item.price || 0) * item.quantity,
      0
    )
  ...
  const productColumns = useMemo<ColumnDef<IncomeProduct>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Badge
            color={getIncomeSourceBadgeColor(row.original.source)}
            variant="light"
            size="sm"
          >
            {getIncomeSourceLabel(row.original.source)}
          </Badge>
        )
      },
      {
